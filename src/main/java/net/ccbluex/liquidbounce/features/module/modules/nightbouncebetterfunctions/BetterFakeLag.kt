...
object BetterFakeLag : Module("BetterFakeLag", Category.NIGHTBOUNCEBETTERFUNCTIONS, gameDetecting = false) {
    ...
    private fun getRandomDelay(): Int {
        val base = delay
        return (base * (0.85 + Math.random() * 0.3)).toInt()
    }
    ...
    val onPacket = handler<PacketEvent> { event ->
        ...
        if (player.health < player.maxHealth && player.hurtTime != 0) {
            blink()
            return@handler
        }
        ...
        when (packet) {
            ...
            is C0EPacketClickWindow, is C0DPacketCloseWindow -> { blink(); return@handler }
            is S08PacketPlayerPosLook, is C08PacketPlayerBlockPlacement, is C07PacketPlayerDigging, is C12PacketUpdateSign, is C19PacketResourcePackStatus -> { blink(); return@handler }
            is S12PacketEntityVelocity -> if (player.entityId == packet.entityID) { blink(); return@handler }
            is S27PacketExplosion -> if (packet.field_149153_g != 0f || packet.field_149152_f != 0f || packet.field_149159_h != 0f) { blink(); return@handler }
        }
    ...
    val onGameLoop = handler<GameLoopEvent> {
        ...
        if (allowedDistToEnemy.endInclusive > 0) {
            ...
            world.playerEntities.forEach { otherPlayer ->
                ...
                val entityMixin = otherPlayer as? IMixinEntity ?: return@forEach
    ...
    val onRender3D = handler<Render3DEvent> { event ->
        ...
        if (line) {
            ...
            for (i in 0 until positions.size - 1) {
                val p1 = positions.elementAt(i).pos
                val p2 = positions.elementAt(i + 1).pos
                val steps = 4
                for (t in 0..steps) {
                    val f = t / steps.toDouble()
                    val interp = p1.lerpWith(p2, f)
                    glVertex3d(interp.xCoord - renderPosX, interp.yCoord - renderPosY, interp.zCoord - renderPosZ)
                }
            }
    ...
    private fun handlePackets(clear: Boolean = false) {
        synchronized(packetQueue) {
            packetQueue.removeEach { (packet, timestamp) ->
                if (timestamp <= System.currentTimeMillis() - getRandomDelay() || clear) {
        ...
        synchronized(positions) {
            positions.removeEach { (_, timestamp) -> timestamp <= System.currentTimeMillis() - getRandomDelay() || clear }
        }
    }
}
